...
import { demoFeed, liveFeed } from "@/lib/feed";
import { quickEvidence } from "@/lib/evidence/build";
import { revenueVsForecast } from "@/lib/demo-data";

const usd = (v: number) => `$${(v / 1e6).toFixed(1)}M`;

export function DfiView({ sdg, sdgLive }: { sdg: SdgIndicator[]; sdgLive: boolean }) {
  const revenue = revenueVsForecast();
  const onTrack = sdg.filter((s) => s.value / s.target >= 0.6).length;
  const sdgFeed = sdgLive ? liveFeed() : demoFeed();
    ...
    <>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <HeroStat
          ...
          feed={sdgFeed}
          evidence={quickEvidence({ metric: "SDG indicators on track", value: `${onTrack} / ${sdg.length}`, objectRef: "object · SDG basket (8/9/10/17)", confidence: sdgLive ? 0.75 : 0.4, status: sdgLive ? "live" : "demo", pillar: "SDG Reporting", source: "un-sdg-api", silver: "sdg_indicators", gold: "gold_sdg_latest", recommendation: "Direct blended finance to the freight-volume (SDG 9.1.2) gap — the largest correctable lag." })}
        />
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-2">
        ...
        <ChartCard
          title="SDG progress to 2030 targets"
          subtitle={sdgLive ? "Live UN SDG data, averaged across pilot countries · targets are illustrative 2030 goals" : "AfCFTA aggregate (demo) · colour = goal (8 blue · 9 teal · 10 amber · 17 violet)"}
